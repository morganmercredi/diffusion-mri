function fit_results = FitOGSEGammaCylinders(signal_file, cylinder_file, SNR)
% FitOGSEGammaCylinder.m is a function that analyses OGSE simulation output and
...
switch model
    case 1
        signal_model = @(par,x) cosOGSEGammaCylinder(par(1:6),x);
        lb = [0 0 0 0 0 0];
...
h = print_axon_distribution(cylinder_file, @gampdf, fit_results.pars(1,1:2)', fit_results.fit_nl_covB, Rmin, Rmax, Rincrements);
